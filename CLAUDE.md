...
Gitignoring cannot fix this on its own: an ignore rule protects the file a tool writes, not the tracked file a tool *edits*. If you need one of these tools while working in this repo, enable it globally or in a scratch clone, not here.

The `leak-guard` job in `.github/workflows/docs.yml` catches the *state files* an enable writes, because they are tracked-and-ignored. It does **not** check this file's `@`-import lines. A validator for those existed briefly and was removed: correctly classifying an `@`-token in markdown means handling fences, indented blocks, inline code spans, and prose mentions, and each rule added to it created a new way to miss a real import or fail the build on a correct line. A dangling import is cosmetic — Claude Code ignores a target that is not there — and a check that cries wolf on valid documentation costs more than the thing it prevents. So this one is on you: if you run an enable here, back out the `@`-import line it appends to this file yourself.
